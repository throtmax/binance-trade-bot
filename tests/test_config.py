...
def test_config_on_run(DoUserConfigEnv):
    ...
    for ikey in params:
        ...
        if ikey=='HOURS_TO_KEEP_SCOUTING_HISTORY': ikey = 'SCOUT_HISTORY_PRUNE_TIME'
        ...
        if ikey=='SCOUT_HISTORY_PRUNE_TIME': mustvalue = float(mustvalue)
        if ikey=='ENABLE_PAPER_TRADING':
            mustvalue = False if mustvalue == '0' or mustvalue.upper() == 'FALSE' else True
